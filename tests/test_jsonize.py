...
def test_default_param():
    ...
    assert obj.to_json(with_defaults=True) == {"type_": "dpt", "literal_d": 0, "array_fixed_d": [None, False, 0, 0.0, ""], "array_d": [], "obj_d": {}}
    ...
    obj = DefaultParamTypes(1, [None, True, 2, 1.0, "string"], [1, 2, 3], {"key": "value"})
    assert obj.to_json() == {"type_": "dpt", "literal_d": 1, "array_fixed_d": [None, True, 2, 1.0, "string"], "array_d": [1, 2, 3], "obj_d": {"key": "value"}}
    assert obj.to_json(with_defaults=True) == {"type_": "dpt", "literal_d": 1, "array_fixed_d": [None, True, 2, 1.0, "string"], "array_d": [1, 2, 3], "obj_d": {"key": "value"}}
    assert ph.dumps(obj) == r'{"type_": "dpt", "literal_d": 1, "array_fixed_d": [null, true, 2, 1.0, "string"], "array_d": [1, 2, 3], "obj_d": {"key": "value"}}'
    assert obj == DefaultParamTypes.Decoder.loads(ph.dumps(obj))
    assert obj == DefaultParamTypes.from_json(ph.dumps(obj))
    assert isinstance(ph.loads(ph.dumps(obj)), DefaultParamTypes)
